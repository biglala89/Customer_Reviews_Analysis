...
database = os.getcwd()+'/{}.db'.format(db_name)
number_of_pages = 10 # Enter an arbitrary number
...
class Scrape_User_Reviews(object):
	def __init__(self, link, database, number_of_pages):
		self.link = link
		self.database = database
		self.number_of_pages = number_of_pages

	# Create connection
	def Create_DB_and_Tables(self, database):
		try:
	...
	# Main function
	def Run(self):
		for num in xrange(1, number_of_pages+1):
			page = link+str(num)
...
if __name__ == "__main__":
	scraper = Scrape_User_Reviews(link, database, number_of_pages)
	t_start = time.time()
	scraper.Create_DB_and_Tables(database)
	scraper.Run()
	t_end = time.time()
